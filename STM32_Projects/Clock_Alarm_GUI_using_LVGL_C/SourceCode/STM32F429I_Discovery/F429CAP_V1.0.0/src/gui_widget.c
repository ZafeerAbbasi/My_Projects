...
# ----------------------------------------------                               #
# Last Modified: Friday, September 22nd 2023, 9:19:01 pm                       #
...
lv_obj_t *createDisplaySlider( lv_obj_t *parent, const char *icon, const char*txt, int32_t min, int32_t max, int32_t currVal, lv_event_cb_t eventCallBack )
{
    ...
    lv_label_set_text_fmt( sliderPercentLabel, "%ld%%", lv_slider_get_value( slider ) );
...
void eventHandlerDisplaySlider( lv_event_t *event )
{
    ...
    lv_label_set_text_fmt( displayLabel, "%ld%%", lv_slider_get_value( displaySlider ) );
}
